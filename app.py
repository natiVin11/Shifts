...
# --- פונקציות עזר (הצפנה, ICS, ניהול נתונים) ---
...
if not st.session_state.auth["logged_in"]:
    ...
    st.markdown('<h1>MGROUP 360 | מערכת ניהול אחודה</h1>', unsafe_allow_html=True)
    ...
    if u_info['role'] == "IT":
        st.title("🛠️ פאנל IT")
        it_tabs = st.tabs(["👥 משתמשים", "📂 ייבוא/ייצוא", "✅ צ'ק-ליסט מערכות", "📊 ביצועים"])
        with it_tabs[0]:
            users = load_data("users")
            ed_u = st.data_editor(users, num_rows="dynamic", use_container_width=True)
    ...
    # --- 2. מנהל מוקד (Manager) ---
        ...
        m_tabs = st.tabs(["📅 עריכת סידור", "📝 אילוצי נציגים", "📈 דוחות BI"])
        with m_tabs[0]:
            sched = load_data("schedule")
            ed_s = st.data_editor(sched[sched['team'] == u_info['team']], num_rows="dynamic")
            if st.button("פרסם סידור"):
                save_data(pd.concat([sched[sched['team'] != u_info['team']], ed_s]), "schedule")
        with m_tabs[1]:
    ...
    # --- 3. ר"צ (Team Lead) ---
        ...
        st.title(f"🚀 פאנל ר\"צ")
        tl_tabs = st.tabs(["👥 צוות", "📅 לוח שנה", "🔮 אילוצים וחיזוי"])
        with tl_tabs[1]:
            sc = load_data("schedule")
            evs = [{"title": f"{r['username']}", "start": r['date'], "end": r['date']} for i, r in sc.iterrows()]
            calendar(events=evs, options={"initialView": "dayGridMonth", "direction": "rtl"})

    # --- 4. נציג (Agent) - 2 מסכים מופרדים ---
    elif u_info['role'] == "נציג":
        st.header(f"👤 פורטל נציג: {u_info['user']}")
        a_tabs = st.tabs(["📝 הגשת אילוצים", "📅 המשמרות שלי"])
        
        with a_tabs[0]:
            st.subheader("בחר יום בלוח השנה להגשת אילוץ")
            col_c, col_f = st.columns([2, 1])
            with col_c:
                res_c = calendar(events=[], options={"initialView": "dayGridMonth", "direction": "rtl"}, key="c_cal")
                if res_c.get("callback") == "dateClick": st.session_state.agent_d = res_c["dateClick"]["date"]
            with col_f:
                st.markdown('<div class="custom-card">', unsafe_allow_html=True)
                d_sel = st.session_state.get('agent_d', str(date.today()))
                st.write(f"תאריך: **{d_sel}**")
                s_t = st.time_input("התחלה", time(8,0))
                e_t = st.time_input("סיום", time(16,0))
                if st.button("שמור אילוץ"):
                    c_df = load_data("constraints")
                    new_c = pd.DataFrame([{"username": u_info['user'], "date": d_sel, "start_time": s_t.strftime("%H:%M"), "end_time": e_t.strftime("%H:%M")}])
                    save_data(pd.concat([c_df, new_c]), "constraints")
                    st.success("נשלח!")
                st.markdown('</div>', unsafe_allow_html=True)

        with a_tabs[1]:
            st.subheader("סידור עבודה מאושר")
            ...
            c_v, c_d = st.columns([2, 1])
            with c_v: calendar(events=ev_sh, options={"initialView": "dayGridMonth", "direction": "rtl"}, key="s_cal")
            with c_d:
                st.write("הורדה ליומן:")
                for i, r in my_sh.iterrows():
                    try:
                        ...
                        st.download_button(f"📅 {r['date']}", create_ics("MGROUP", s_dt, e_dt), file_name="shift.ics", key=f"dl_{i}")
                    except: pass

    # --- 5. משא"בי אנוש (HR) ---
    elif u_info['role'] == "משא":
        st.title("📋 פורטל HR")
        hr_tabs = st.tabs(["Lifecycle", "דוחות"])
        with hr_tabs[0]:
            nm = st.text_input("שם עובד")
            act = st.radio("פעולה", ["קליטה", "גריעה"])
            if st.button("שלח ל-IT"):
                onb = load_data("onboarding")
                save_data(pd.concat([onb, pd.DataFrame([{"username": nm, "type": act, "status": "ממתין", "date": str(date.today())}])]), "onboarding")
